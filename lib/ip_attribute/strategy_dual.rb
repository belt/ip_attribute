  ...
  module StrategyDual
    ...
    def _install_dual_accessors(prefix)
      ...
      define_method(:"#{ipv4_col}=") do |addr|
        parsed = _ip_parse(addr)
        write_attribute(ipv4_col, _ipv4_int(parsed))
      end

      define_method(:"#{ipv6_col}=") do |addr|
        parsed = _ip_parse(addr)
        write_attribute(ipv6_col, _ipv6_int(parsed))
      end
    end
    ...
    def _install_dual_display(prefix)
      ...
      define_method("#{prefix}_ip_display") do
        ipv4_val = public_send(ipv4_col)
        ipv6_val = public_send(ipv6_col)
        parts = [ipv4_val&.to_s, ipv6_val&.to_s].compact
