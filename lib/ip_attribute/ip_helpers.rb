...
module IpAttribute
  ...
  module IpHelpers
    ...
    def _ipv4_int(ip_addr)
      IpParser.ipv4_int(ip_addr)
    end

    # Extract IPv6 integer (rejects mapped — those go to _ipv4).
    def _ipv6_int(ip_addr)
      IpParser.ipv6_int(ip_addr)
    end
  end

  # Extracted parser class to avoid UtilityFunction warnings
  class IpParser
    def self.parse(addr)
